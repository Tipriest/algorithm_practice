...
m = np.stack((points1, points2), axis=1)
rr.log(
...
beads = [
    bounce_lerp(points1[i], points2[i], offsets[i]) for i in range(NUM_POINTS)
]
colors = [
    [int(bounce_lerp(80, 230, offsets[i] * 2))] for i in range(NUM_POINTS)
]
x = np.repeat(colors, 3, axis=-1)
rr.log(
...
for i in range(400):
    time = i * 0.01
    rr.set_time_seconds("stable_time", seconds=time)
    # rr.set_time("stable_time", duration=time)
    times = np.repeat(time, NUM_POINTS) + time_offsets
    beads = [
        bounce_lerp(points1[i], points2[i], times[i]) for i in range(NUM_POINTS)
    ]
    colors = [
